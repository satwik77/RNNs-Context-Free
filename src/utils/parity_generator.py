...
class ParityLanguage():
	...
	'''
	def generate_string(self, maxlength, num_ones = 3):
		num_ones = np.random.randint(0, num_ones + 1)
		num_zeros = np.random.randint(0, maxlength - num_ones)
		string  = ['1' for _ in range(num_ones)] + ['0' for _ in range(num_zeros)]
	...
	def generate_list(self, num, min_length, max_length):
		...
		while len(arr) < num:
			...
			if len(string) >= min_length and len(string) <= max_length:
				...
				print("Generated {}/{} strings".format(len(arr), num), end = '\r', flush = True)
		return arr
