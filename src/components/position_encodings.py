...
class PositionalEncoding(nn.Module):
	...
	def __init__(self, d_model, dropout=0.1, max_period = 10000.0, max_len=1400):
...
class CosineNpiPositionalEncoding(nn.Module):
	...
	def __init__(self, d_model, dropout=0.1 , max_len=5000, factor = 2.0):
		...
		for i in range(max_len):
			pe[i] = pe[i] * math.cos(i * math.pi / factor)
		pe = pe.unsqueeze(0).transpose(0, 1)
		#pdb.set_trace()
		self.register_buffer('pe', pe)
