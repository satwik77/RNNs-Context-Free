...
from src.utils.logger import get_logger, print_log, store_results, store_test_results
	...
	if config.mode == 'train':
		...
		if config.lang == 'Dyck':
			if config.generate:
				...
				data_dir = os.path.join(data_path, config.dataset)
				with open(os.path.join(data_dir, '{}_corpus_bins.pk'.format(config.test_prefix)), 'rb') as f:
...
def main():
	...
	if config.results:
		if mode == 'train':
			...
			config.result_path = os.path.join(result_folder, 'test_results_{}.json'.format(config.dataset))
	if is_train:
	...
	if is_train:
		...
		with open(config_file, 'rb') as f:
			...
			config.batch_size = 1
			config.bias = bias
			config.tune_threshold = tune_threshold
			config.test_prefix = test_prefix
			config.result_path = os.path.join(result_folder, '{}_results_{}.json'.format(config.test_prefix,config.dataset))
			try:
				config.pos_encode_type
			except:
		...
		if not config.tune_threshold:
			...
			for i in range(config.bins):
				...
				test_acc_bins.append(test_acc_epoch)
			store_test_results(config, test_acc_bins)
			logger.info("Analysis results written to {}...".format(result_folder))

		else:
			epsilons = np.linspace(0.1, 0.9, 9)
			for epsilon in epsilons:	
				logger.info("Testing for Epsilon = {}".format(epsilon))
